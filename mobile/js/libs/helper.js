(function (a) {
        ...
        if (!location.hash || !e.addEventListener) {
            ...
            var c = 1
                , b = setInterval(function () {
                    if (d.body) {
                        clearInterval(b);
                        c = "scrollTop" in d.body ? d.body.scrollTop : 1;
    ...
    MBP.fastButton = function (b, c) {
        ...
        ;
    MBP.fastButton.prototype.handleEvent = function (b) {
        switch (b.type) {
            case "touchstart":
                this.onTouchStart(b);
                break;
    ...
    MBP.fastButton.prototype.onTouchStart = function (b) {
        ...
        ;
    MBP.fastButton.prototype.onTouchMove = function (b) {
        if (Math.abs(b.touches[0].clientX - this.startX) > 10 || Math.abs(b.touches[0].clientY - this.startY) > 10) {
            this.reset()
        }
    }
        ;
    MBP.fastButton.prototype.onClick = function (b) {
        b.stopPropagation();
        this.reset();
        this.handler(b);
        if (b.type == "touchend") {
    ...
    MBP.fastButton.prototype.reset = function () {
        ...
        ;
    MBP.preventGhostClick = function (b, c) {
        MBP.coords.push(b, c);
    ...
    MBP.ghostClickHandler = function (e) {
        for (var d = 0, c = MBP.coords.length; d < c; d += 2) {
            var b = MBP.coords[d];
            var f = MBP.coords[d + 1];
            if (Math.abs(e.clientX - b) < 25 && Math.abs(e.clientY - f) < 25) {
                e.stopPropagation();
                e.preventDefault()
            }
        }
    ...
    MBP.splash = function () {
        ...
        ;
    MBP.autogrow = function (d, b) {
        function e(h) {
            var g = this.scrollHeight
                , i = this.clientHeight;
            if (g > i) {
                this.style.height = g + 3 * f + "px"
            }
        }
        var c = (b) ? b : 12
            , f = d.currentStyle ? d.currentStyle.lineHeight : getComputedStyle(d, null).lineHeight;
        f = (f.indexOf("px") == -1) ? c : parseInt(f, 10);
        d.style.overflow = "hidden";
        d.addEventListener ? d.addEventListener("keyup", e, false) : d.attachEvent("onkeyup", e)
    }
}
)(document);